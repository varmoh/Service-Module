...
const RegexDetail: FC = () => {
  ...
  const regexData = useMemo(() => regex && regex.examples.map((e, index) => ({ id: index, value: e })), [regex]);
      ...
      columnHelper.accessor("value", {
        ...
        cell: (props) =>
          editableRow && editableRow.id === props.row.original.id ? (
            <FormInput name={`example-${props.row.original.id}`} label="" defaultValue={editableRow.value} hideLabel />
          ) : (
            props.getValue()
          ),
      }),
          ...
          <>
            {editableRow && editableRow.id === props.row.original.id ? (
              <Button
                appearance="text"
                onClick={() =>
                  regexExampleEditMutation.mutate({
    ...
    <>
              ...
              <Track justify="between">
                <Track gap={16}>
                  ...
                  {editingRegexTitle ? (
                    <Button
                      appearance="text"
                      onClick={() => regexEditMutation.mutate({ id: regex.id, data: { name: editingRegexTitle } })}
                    >
                      <Icon icon={<MdOutlineSave />} />
                      {t("global.save")}
                    </Button>
                  ) : (
                    <Button appearance="text" onClick={() => setEditingRegexTitle(regex.name)}>
                ...
                <p style={{ color: "#4D4F5D" }}>
                  {`${t("global.modifiedAt")} ${format(new Date(regex.modifiedAt), "dd.MM.yyyy")}`}
                </p>
              </Track>
              <Track justify="end" gap={8}>
                <Button appearance="secondary" onClick={() => handleRegexExamplesUpload(regex.id)}>
                  {t("training.intents.upload")}
                </Button>
                <Button appearance="secondary" onClick={() => handleRegexExamplesDownload(regex.id)}>
                  {t("training.intents.download")}
                </Button>
                <Button appearance="error" onClick={() => setDeletableRegex(regex.id)}>
                  {t("global.delete")}
                </Button>
              </Track>
            </Track>
      ...
      {deletableRegex !== null && (
        <Dialog
          ...
          footer={
            <>
              <Button appearance="secondary" onClick={() => setDeletableRegex(null)}>
                {t("global.no")}
              </Button>
              <Button appearance="error" onClick={() => regexDeleteMutation.mutate({ id: deletableRegex })}>
                {t("global.yes")}
              </Button>
            </>
          }
        ...
        <Dialog
          ...
          footer={
            <>
              ...
              <Button appearance="error" onClick={() => regexExampleDeleteMutation.mutate({ id: deletableRow })}>
                {t("global.yes")}
              </Button>
            </>
          }
